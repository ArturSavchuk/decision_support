...
namespace Lab3
{
    class Program
    {
        ...
        public static CriteriaRelation ReadCriteriaRelation()
        {
            ...
            string fileName = $"{directoryPath}\\Lab3\\relations_var52.txt";
            ...
            HashSet<int> criteriasImportance =
                new HashSet<int> { 10,11,5,8,1,9,4,7,2,12,6,3 }.Select(c => c - 1).ToHashSet();
            List<HashSet<int>> criteriasImportancesClasses =
                new List<HashSet<int>>
                {
                    new HashSet<int> { 2, 4, 7 }.Select(c => c - 1).ToHashSet(),
                    new HashSet<int> { 6, 9}.Select(c => c - 1).ToHashSet(),
                    new HashSet<int> { 1, 3, 5, 8, 10, 11, 12}.Select(c => c - 1).ToHashSet()
                };
        ...
        public static void PrintRelation<T>(Relation relation, Func<T[][]> printingSelector)
        {
            ...
            Console.WriteLine();
            Console.WriteLine($"{string.Join(' '.ToString(), relation)}");
            Console.WriteLine();
        ...
        public static void PrintCriteriaRelationVectors(CriteriaRelation criteriaRelation, Func<List<int>[][]> printingSelector)
        {
            ...
            for (int i = 0; i < criteriaRelation.AlternativesCount; i++)
            {
                ...
                {
                    Console.WriteLine($"[{i}][{j}]: {string.Join(' '.ToString(), toPrint[i][j])}");
                }
            }
            Console.WriteLine();
        }
